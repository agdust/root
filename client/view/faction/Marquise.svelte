...
let scale, card, craftedItems, tile, wood;
...
<div class='container'>
  <div class='board' style={`width: ${2252 * scale}px; height: ${1749 * scale}px`}>
    ...
    {#each new Array($game.factionData.marquise.workshop).fill(0) as _, i}
      <Piece piece={Pieces.marquise.workshop} x={tile.x - tile.dx * i} y={tile.y + tile.dy}
             {scale}/>
    {/each}
    {#each new Array($game.factionData.marquise.recruiter).fill(0) as _, i}
      <Piece piece={Pieces.marquise.recruiter} x={tile.x - tile.dx * i} y={tile.y + 2 * tile.dy}
             {scale}/>
    {/each}
    <Piece piece={Pieces.marquise.wood} x={wood.x} y={wood.y} {scale}
